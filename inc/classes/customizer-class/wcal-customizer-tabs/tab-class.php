...
if(!class_exists('WCAL_Customize_Control_Tabs')){
	class WCAL_Customize_Control_Tabs extends WP_Customize_Control {

		/**
		 * WCAL_Customize_Control_Tabs constructor.
		 *
		...
		public function render_content() {
			...
			foreach ( $this->tabs as $value => $args ) {
				if ( ! empty( $args['controls'] ) && ( $this->tab_has_controls( $args['controls'] ) ) ) {
					...
					$output .= '</label>';
					$output .= '</div><!-- /.wcal-customizer-tab -->';
				}
			}
			$output .= '</div><!-- /.wcal-tabs-control -->';
		...
		public function enqueue() {
			...
			wp_enqueue_script( 'wcal-tabs-control-script', WCAL_JS_DIR . '/wcal-script.js', array( 'jquery' ), '15258', true );
			wp_enqueue_style( 'wcal-tabs-control-style', WCAL_CSS_DIR. '/wcal-tab-style.css', null, '15258' );

		}

		/**
		 * Enqueue the partials handler script that works synchronously with the wcal-tabs-control-script
		 */
		public function partials_helper_script_enqueue() {
			wp_enqueue_script( 'wcal-tabs-addon-script', WCAL_JS_DIR . '/wcal-addons.js', array( 'jquery' ), '15258', true );
		}
